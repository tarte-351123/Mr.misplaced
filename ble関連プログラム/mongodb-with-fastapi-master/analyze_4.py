...
filename = 'ble関連プログラム/csv/go_out01.csv'
...
for i in range(24):
    for j in range(6):
        ...
        bb.append(dt.strptime(time[i][j],'%H:%M'))
        aa.append(time_count_d[i][j])
print(bb[0])

        
bx = plt.subplot()
bx.bar(bb,aa,width=0.005)     
bx.xaxis.set_major_locator(mdates.HourLocator(interval=4))
bx.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
bx.set_xlim([dt.strptime('00:00','%H:%M'), dt.strptime('23:59','%H:%M')])
